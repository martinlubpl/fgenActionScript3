...
if ($action == "unassignvideo") {
	$query = "UPDATE fg_movies SET model_id=0 WHERE id=".$_GET['vid'];
	$result = mysql_query($query) or die(mysql_error());
}

if ($action == "assignvideo") {
	$query = "UPDATE fg_movies SET model_id=".$_GET['id']." WHERE id=".$_GET['vid'];
	$result = mysql_query($query) or die(mysql_error());
}
...
if ($action=="edit" || $action == "addphoto" || $action == "photoflags" || $action=="delete_photo" || $action=="update" || $action == "addpdf" || $action == "updatepdf" || $action == "delete_sc" || $action=="assignvideo" || $action == "unassignvideo") {
	...
	} elseif ($action == "assignvideo") {
		$modelId = $_GET['id'];
	} elseif ($action == "unassignvideo" ) {
		$modelId = $_GET['id'];
	}
...
?>
          ...
          <b>VIDEOS:</b> <br><br>
          <?
          $query = "SELECT * FROM fg_movies WHERE model_id=".$modelId." ORDER BY rank ASC";
          $result = mysql_query($query) or die(mysql_error());

          if (mysql_num_rows($result)>0) {
          	?>
          	<ul class="sortable-list" id="videos_list">
          		<?
          		while ($got = mysql_fetch_array($result)) {
          			$imgsrc =  substr($got['path'],0,strlen($got['path'])-3)."jpg";
          			?>
          			<li id="video_<?=$got['id']?>" class="sortable-list">
          			<table>
          			<tr>
          			<td><A target=\"_blank\" HREF=watchvideo.php?myurl=../movies/<?=$got['path']?>><img height="50%" width="50%" src="../movies/<?=$imgsrc?>"></a></td>
          			<td><?=$got['path']?></td>
          			<td><a href="models.php?action=unassignvideo&id=<?=$modelId?>&vid=<?=$got['id']?>"><img src="gfx/btn_delete.gif"></a></td>
          			</tr>
          			</table>
          			</li>
          			<?
          	...
          	<script type="text/javascript">
          	function updateOrderVideo()
          	{
          		var options = {
          			method : 'post',
          			parameters : Sortable.serialize('videos_list') + '&model_id=<?=$modelId?>'
          		};
          		//alert(Sortable.serialize('videos_list') + '&model_id=<?=$modelId?>');
          		
          		new Ajax.Request('processorvideo.php', options);
          	}
          	Sortable.create('videos_list', { onUpdate : updateOrderVideo });
        </script>
          	<?
          	print "<br><br><br>";
          } else {
          	print "THIS MODEL HAS NO VIDEOS<br><br><br>";
          }
          ...
          <?
          /// LOOK FOR UNASSIGNED VIDEOS
          $query = "SELECT * FROM fg_movies WHERE model_id=0";
          $result = mysql_query($query) or die(mysql_error());

          if (mysql_num_rows($result)>0) {
	?>
	
	There are some new videos uploaded and not assigned to any model. If you want to assign any video to this model click the corresponding thumbnail below:<br>
	<?
	while ($got = mysql_fetch_array($result)) {
		$imgsrc =  substr($got['path'],0,strlen($got['path'])-3)."jpg";


		?><a href="models.php?vid=<?=$got['id']?>&action=assignvideo&id=<?=$modelId?>"><img height="85" width="61" border="0" hspace="10" src="../movies/<?=$imgsrc?>"></a><?
	}
