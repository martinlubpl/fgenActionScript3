 ...
 */
	...
	foreach($uploadExtArr as $ext) {
		if (in_array($ext, $fileSysExtArr))
			$validExtArr[] = $ext;
	}
	...
	for ($i=0; isset($_FILES['file' . $i]['tmp_name']); $i++) {
		...
		if (is_uploaded_file($_FILES['file' . $i]['tmp_name'])) {
			...
			if ($file->exists()) {
				...
				$data['errorMsg'] = "Error: File by that name allready exists.";
			...
			if ($toBig)
				$msg = "The file you are trying to upload is to large. The max allowed file size is: " . $maxSize . $prefix;
